-- Add a dedicated Telegram destination for sanitized operational incidents.
-- Apply after 015. This migration contains no bot token, chat ID, or secret.

SET search_path TO ticket_cluster, public;

ALTER TABLE telegram_connections
  DROP CONSTRAINT IF EXISTS telegram_connections_role_check;
ALTER TABLE telegram_connections
  ADD CONSTRAINT telegram_connections_role_check
  CHECK (role IN ('review', 'engineering', 'operations'));

ALTER TABLE telegram_setup_codes
  DROP CONSTRAINT IF EXISTS telegram_setup_codes_role_check;
ALTER TABLE telegram_setup_codes
  ADD CONSTRAINT telegram_setup_codes_role_check
  CHECK (role IN ('review', 'engineering', 'operations'));
...
DECLARE v_code text;
BEGIN
  IF p_role NOT IN ('review', 'engineering', 'operations') THEN
...
BEGIN
  IF p_role NOT IN ('review', 'engineering', 'operations') THEN RETURN 'invalid_role'; END IF;
