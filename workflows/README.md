...
The n8n implementation is split into credential-free imports. Choose exactly one operational monitor for each deployment interface:
...
8. `operational-monitor.template.json` — Slack operations monitor; refreshes incidents and posts a rate-limited alert to `#triage-operations`.
9. `operational-monitor.telegram.template.json` — Telegram operations monitor; uses the same incident ledger and posts to the connected Operations group with the existing bot.
...
- Import **Operational error capture** and select it as the Error Workflow in each production workflow's Settings. It records only sanitized execution metadata, never ticket text or credentials. For Slack, activate `operational-monitor.template.json` with Slack and Postgres credentials. For Telegram, apply migration `016`, connect the Operations group, and activate `operational-monitor.telegram.template.json` with Telegram and Postgres credentials. Never activate both monitors for the same deployment because they share one notification ledger.
...
- For Telegram, create a native **Telegram API** credential backed by `TELEGRAM_BOT_TOKEN` and attach it to every Telegram node. Expose `TELEGRAM_WEBHOOK_SECRET` only to the n8n container for the custom webhook Code node. Exported workflows contain neither value. Follow `docs/telegram-interface-runbook.md` to connect the private review, engineering, and operations groups with expiring one-time codes.

## Safe import order and activation test

1. Apply all SQL migrations in numeric order to staging first, including `016` for Telegram operations.
